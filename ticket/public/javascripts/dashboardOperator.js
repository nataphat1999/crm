...
async function getUser(department_id = "") {
    $("#user").select2({
    ...
    try {
        ...
        setUserSelection(response.data.Result);
    } catch (err) {
        Swal.fire({
            icon: "error",
            title: "Oops...",
...
function setUserSelection(data) {
    var ticket_user_selected = document.getElementById("user");

    console.log(data);

    ticket_user_selected.innerHTML = "";
    var optAll = document.createElement("option");
    optAll.value = "";
    optAll.innerHTML = "All";
    optAll.selected = true;
    ticket_user_selected.appendChild(optAll);
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        ticket_user_selected.appendChild(opt);
    }
}
...
function getTicket(status_id) {
    ...
    var user_type = 'operator';
    const department_id = $("#department").val();
    const user_id_selected = $("#user").val();
    const start_date = $("#startDate").val();
    const end_date = $("#endDate").val();
    const project_code = $("#project_code").val();
    const customer_crm = $("#customer_crm").val();
...
function setTotalTicket(user_type, data, status_id) {
    ...
    $("#total_monthly_ticket_dev").html(data.Result[0]);
    $("#total_waiting_dev").html(data.Result[1]);
    $("#total_in_process_dev").html(data.Result[2]);
    $("#total_waiting_manager_approve_dev").html(data.Result[3]);
    $("#total_sucess_dev").html(data.Result[4]);
    $("#total_disapprove_dev").html(data.Result[5]);
    $("#total_cancel_dev").html(data.Result[6]);
...
function setDataChart(user_type, data) {
    ...
    var data_labels = [
        '', ['Waiting Operator \n', 'confirm : ' + data.Result[1]],
        'In Process : ' + data.Result[2],
        ['Waiting Approver \n', 'approve : ' + data.Result[3]],
        'Success : ' + data.Result[4],
        'Disapprove : ' + data.Result[5],
        'User Cancel : ' + data.Result[6],
    ...
    for (let i = 0; i < data.Result.length; i++) {
        ...
        if (status_id == 2) {
            ...
            var action = "<div class='row text-center'>" +
                "<div class='col-4'>" +
                "<a href='/ticketDetailOperator?ticket_id=" + ticket_id + "'><span class='badge text-white' style='border-radius: 10px; background-color: #7B61FF;'><img class='card-icon' src='../public/images/icon/google-docs.png' /></span></a>" +
                "</div>" +
                "</div>";
        } else if (status_id == 3) {
            ...
            var status = "<span class='badge text-white' style='border-radius: 10px; background-color: #7F5A58;'>Waiting Approver approve</span>";
            var action = "<div class='row text-center'>" +
                "<div class='col-4'>" +
                "<a href='/ticketDetailOperator?ticket_id=" + ticket_id + "'><span class='badge text-white' style='border-radius: 10px; background-color: #7B61FF;'><img class='card-icon' src='../public/images/icon/google-docs.png' /></span></a>" +
                "</div>" +
            ...
            var status = "<span class='badge text-white' style='border-radius: 10px; background-color: #FB95FB;'>Disapprove</span>";
            var action = "<div class='row text-center'>" +
                "<div class='col-4'>" +
                "<a href='/ticketDetailOperator?ticket_id=" + ticket_id + "'><span class='badge text-white' style='border-radius: 10px; background-color: #7B61FF;'><img class='card-icon' src='../public/images/icon/google-docs.png' /></span></a>" +
                "</div>" +
            ...
            var action = "<div class='row text-center'>" +
                ...
                "</div>" +
                "</div>";
        } else if (status_id == 1 || status_id == 7) {
            var status = "-";
            var action = "-";
...
function cancelTicket(ticket_id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .then(response => lineNotify(response.data.ticket_no, response.data.ticket_name, status_id, response.data.user_firstname, response.data.user_lastname, response.data.user_email, response.data.dev_name))
                .catch(err => Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: err
