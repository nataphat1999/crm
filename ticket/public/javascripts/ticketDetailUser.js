...
function setTypeSelection(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        //console.log(data)
        var ticket_type_id = data[i].ticket_type_id;
        var ticket_type_name = data[i].ticket_type_name;
        var opt = document.createElement("option");
...
function changeAttachFile() {
    $("#ticket_attachment").on("change", function(event) {
        ...
        for (let i = 0; i < attachUpdateFile.length; i++) {
            if (i == 0) {
                $("#files_selected_reject").append('<ul class="ul_files_reject">');
            }
    ...
    $("#ticket_attachment_reject_approver").on("change", function(event) {
        $("#files_selected_reject_approver").empty();
        var files = $("#ticket_attachment_reject_approver").get(0).files;
        var data = Array.from(files);
        ...
        for (let i = 0; i < attachUpdateFile.length; i++) {
            if (i == 0) {
                $("#files_selected_reject_approver").append(
            ...
            if (typeof attachUpdateFile[i] === "string") {
                ...
                list_files =
                    '<li class="li_files_update_reject_approver' +
                    i +
                    '"data-index="' +
                    i +
                    '"><div">' +
            ...
            $("#files_selected_reject_approver").append(list_files);

            if (i == files.length - 1) {
                $("#files_selected_reject_approver").append("</ul>");
            }
        }
        event.target.value = null;
    });
...
function deletefile(element) {
    ...
    if (classList.startsWith("li_files_reject")) {
        attachUpdateFile.splice(index, 1);
        $('[class^="li_files_reject"]').each(function(i) {
            $(this).attr("data-index", i);
        });
    } else if (classList.startsWith("li_files_update_reject_approver")) {
        attachUpdateFile.splice(index, 1);
        $('[class^="li_files_update_reject_approver"]').each(function(i) {
            $(this).attr("data-index", i);
        });
    }
}
...
async function setTicketDetail(data) {
    ...
    if (data.ticket_remark_approver_change_due_date) {
        ...
        if (data.ticket_attachment_approver_change_due_date) {
            ...
            var attachDDFile = attachDDFile = data.ticket_attachment_approver_change_due_date.split("|");
    ...
    if (data.reject_desciption) {
        $("#form_areject_description").show();
        $("#reject_adescription").val(data.reject_desciption);
        if (data.reject_attachment) {
            console.log("TESTTTT", data.reject_attachment)
            ...
            console.log("TESTTTT222", data.reject_attachment)
            $("#form_file_approver_reject").hide();
        }
    }
    ...
    if (
        data.status_id == 10 &&
        (data.ticket_rating_user == 0 || !data.ticket_rating_user)
    ) {
        setRatingUser();
    }
    ...
    if (data.status_id == 1) {
        ...
        $("#btn_rating_user").show();
        $("#selected_approve").show();
        ...
        if (data.reject_user_attachment) {
            $("#file_user_reject_to_approver").show();
...
function addRatingUser() {
    ...
    $selectAll.on("change", function() {
        if ($(this).val() == "Approve") {
            ...
            $("#textarea_note_disapprove").show();
            $("#add_file_reject_approver").show();
            $("#selected_approve_star_user").hide();
        }
    });

    $("#btn_rating_user").click(async function(e) {
        var ticket_id = getUrlParameter("ticket_id");
        var rating_score = $(".scoreNow2").html();
        var update_dev = "";
        var status_id = 6;
        var ticket_department_id = $("#ticket_department_selected").val();
        var ticket_reject_approver_note = $("#ticket_reject_approver_note").val();
        ...
        if ($("#Approve").is(":checked") || $("#Reject").is(":checked")) {
            if ($("#Approve").is(":checked")) {
                status_id = 6;
                msg = "Yes, complete ticket!";
            } else if ($("#Reject").is(":checked")) {
                status_id = 11;
                msg = "Yes, reject ticket!";
            }
        ...
        if (!$("#Approve").is(":checked") && !$("#Reject").is(":checked")) {
            Swal.fire({
                icon: "warning",
                title: "Warning!",
                text: "Please select Approve / Reject",
            });
        } else if (status_id == 6 && rating_score == 0) {
            Swal.fire({
                icon: "warning",
                title: "Warning!",
            ...
            status_id == 11 &&
            ticket_reject_approver_note == "" &&
            attachUpdateFile.length == 0
        ) {
            Swal.fire({
                icon: "warning",
                title: "Warning!",
                text: "Please enter file or Note Reject",
            });
        } else {
            if (attachUpdateFile.length) {
                for (let i = 0; i < attachUpdateFile.length; i++) {
                    filetype = attachUpdateFile[i].name.split(/\.(?=[^\.]+$)/);
                    if (i != 0) {
                        fileAttach += "|";
                        fileAttachType += "|";
                        fileName += "|";
                    }
                    fileAttach += await toBase64(attachUpdateFile[i]);
                    fileAttachType += filetype[1];
                    fileName += filetype[0];
                }
            }
                ...
                if (result.isConfirmed) {
                    ...
                    axios
                        .post("/api/updateRatingUser", {
                            ...
                            rating_score: rating_score,
                            ticket_department_id: ticket_department_id,
                            ticket_attachment_reject: fileAttach,
                            fileAttachType_reject: fileAttachType,
                            fileName_reject: fileName,
                            ticket_reject_approver_note,
                        })
                        .then((response) => {
                            addSuccess();
...
function lineNotify(
    ...
    axios
        .post("/api/LineNotify", {
            ...
            ticket_estimate_date: due_date_manager,
        })
        .then((response) => {})
        .catch((err) =>
            Swal.fire({
